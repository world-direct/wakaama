 ...
 *45.5723  [Worlds Geodetic System 1984].|
 *  Longitude   |  1  |  R   | Single  | Yes | Float   |       |  Deg  | The decimal notation of longitude e.g. -
 *153.21760 [Worlds Geodetic System 1984].|
 *  Altitude    |  2  |  R   | Single  | No  | Float   |       |   m   | The decimal notation of altitude in meters
 *above sea level.                      |
 *  Radius      |  3  |  R   | Single  | No  | Float   |       |   m   | The value in the Radius Resource indicates the
 *size in meters of a circular area |
 *              |     |      |         |     |         |       |       | around a point of geometry. |
 *  Velocity    |  4  |  R   | Single  | No  | Opaque  |       |   *   | The velocity of the device as defined in 3GPP
 *23.032 GAD specification(*).       |
 *              |     |      |         |     |         |       |       | This set of values may not be available if the
 *device is static.                 |
 *              |     |      |         |     |         |       |       | opaque: see OMA_TS 6.3.2 |
 *  Timestamp   |  5  |  R   | Single  | Yes | Time    |       |   s   | The timestamp when the location measurement was
 *performed.                       |
 *  Speed       |  6  |  R   | Single  | No  | Float   |       |  m/s  | Speed is the time rate of change in position of
 *a LwM2M Client without regard    |
 *              |     |      |         |     |         |       |       | for direction: the scalar component of
 *velocity.                                 |
 */
...
/**
  ...
  */
void location_setLocationAtTime(
    lwm2m_object_t *locationObj, float latitude, float longitude, float altitude, uint64_t timestamp)
{
    //-------------------------------------------------------------------- JH --
    location_data_t *pData = locationObj->userData;
